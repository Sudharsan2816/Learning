...
export const curriculum: Lesson[] = [
  {
    ...
    developerExplanation: 'AI is the umbrella. ML is statistical pattern learning from data. Deep learning is representation learning with neural networks. LLMs are transformer-based sequence models trained with self-supervised next-token prediction and often aligned with instruction tuning/RLHF. In software terms: you replace hand-coded decision logic with a model artifact trained from data, then wrap it in evaluation, APIs, monitoring, and product logic.', funExplanation: "Think of AI like the Avengers universe. AI is the whole cinematic universe, ML is the team that learns from missions, deep learning is the tech-heavy Iron Man suit with many layers, and LLMs are the Doctor Strange-style language specialists who predict the next move in a conversation. The point is knowing which hero to call for which problem.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Linear regression models a target y as a weighted sum of input features: y_hat = w·x + b. Training chooses weights that minimize prediction error, commonly mean squared error. It is simple, fast, interpretable, and a strong baseline for continuous prediction problems.', funExplanation: "Linear regression is like Tanjiro in Demon Slayer learning the cleanest sword path through scattered targets. The dots are past examples, the line is the breathing form, and prediction is estimating where the next target will land using the simplest smooth slash.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'A loss function maps model parameters to an error value. Gradient descent computes partial derivatives of the loss with respect to parameters and updates parameters in the negative-gradient direction. Learning rate controls step size.', funExplanation: "Gradient descent is like Naruto training a new jutsu: try it, see how badly it fails, adjust chakra control a little, repeat thousands of times. The loss is Kakashi saying how wrong the move was; the gradient tells which direction to improve.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'A neural network composes affine transformations and nonlinear activations. Forward pass computes predictions. Backpropagation applies the chain rule to compute gradients of loss with respect to every parameter, enabling gradient-based optimization.', funExplanation: "A neural network is like the Straw Hat crew in One Piece passing information across the ship. Each member transforms the message a bit, and backpropagation is the after-battle review where everyone learns who needs to adjust their role so the next mission works better.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Transformer LLMs tokenize input, map tokens to embeddings, apply stacked self-attention and feed-forward layers, and output a probability distribution over the vocabulary for the next token. Generation samples/decodes from that distribution repeatedly.', funExplanation: "An LLM is like Sherlock Holmes reading a sentence clue by clue. Tokens are clues, embeddings are each clue's meaning map, attention is deciding which clues matter right now, and the transformer is the detective board connecting everything before guessing the next word.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'LLM apps coordinate model calls with retrieval, structured outputs, function/tool schemas, orchestration graphs, memory/state, and external protocols. LangGraph models stateful agent workflows as graphs; Semantic Kernel organizes skills/plugins and planners; MCP standardizes tool/resource connections between models and systems.', funExplanation: "Building LLM apps is like assembling an Avengers mission control room. Prompts give the mission brief, RAG brings the archive, tools are specialist gadgets, agents decide the next action, and MCP/LangGraph/Semantic Kernel are the coordination rules so nobody improvises dangerously.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Production AI combines ML evaluation, data/version control, CI/CD, observability, system design, security, reliability engineering, and cost-performance optimization. LLM systems add prompt/version management, eval sets, guardrails, rate limits, latency budgets, and human feedback loops.', funExplanation: "Production AI is like running Jurassic Park after the dinosaurs already work. The impressive model is only one part; you still need fences, alarms, monitoring, emergency protocols, cost control, and safety reviews before real visitors arrive.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Features are represented as vectors, datasets as matrices, and model layers as matrix transformations. Dot products measure alignment/similarity, matrix multiplication batches many dot products, and eigenvectors describe directions preserved by a transformation.', funExplanation: "Linear algebra is like the 3D maneuver gear in Attack on Titan. Vectors say direction and strength, matrices transform movement, dot products measure alignment, and eigenvectors are directions that keep their identity even after a transformation hits.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Probability models uncertainty; statistics estimates population properties from samples. ML uses distributions, expectation, variance, likelihood, confidence intervals, hypothesis tests, and Bayesian thinking to train/evaluate models and experiments.', funExplanation: "Probability is like Doctor Strange checking possible futures. You rarely know one certain outcome, so you reason with likelihoods, evidence, averages, uncertainty, and confidence before choosing the smartest move.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Binary classifiers estimate P(y=1|x). Logistic regression uses a linear score z=w·x+b passed through sigmoid. Threshold choice converts probabilities into labels; metrics depend on business costs and class imbalance.', funExplanation: "Classification is like the Sorting Hat from Harry Potter assigning each student to a house. Logistic regression gives a probability, the threshold makes the final call, and metrics check whether the hat is fair, accurate, or making costly mistakes.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Trees recursively partition feature space by impurity reduction. Random forests reduce variance via bagging and feature randomness. Gradient boosting fits weak learners to residuals/gradients, producing strong tabular predictors.', funExplanation: "Decision trees are like a Choose Your Own Adventure movie: each question sends you down a branch. Random forests are a council of many such stories voting together, and boosting is the training montage where each new tree focuses on what the previous ones messed up.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Unsupervised methods infer structure without labels. K-means minimizes within-cluster distances; PCA finds orthogonal directions of maximum variance; feature engineering encodes domain assumptions into model inputs.', funExplanation: "Unsupervised learning is like L from Death Note organizing suspects without labels. Clustering groups similar behavior, PCA compresses the evidence board to the most important axes, and feature engineering decides which clues are worth pinning up.",
    jargon: [
  ...
  },
  {
    ...
    developerExplanation: 'Convolutional neural networks use shared filters over spatial neighborhoods, preserving locality and reducing parameters compared with dense layers. Pooling/downsampling builds some translation tolerance.', funExplanation: "CNNs are like Spider-Man scanning a city block. Small filters notice edges and textures, deeper layers recognize objects and scenes, and pooling keeps the important signal while ignoring tiny shifts in where something appears.",
    jargon: [
